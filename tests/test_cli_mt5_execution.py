...
def test_mt5_run_once_invokes_runner(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(cli_main.DEFAULT_CONFIG, "runner_loss_streak_cooldown_count", 2)
    monkeypatch.setitem(cli_main.DEFAULT_CONFIG, "runner_loss_streak_cooldown_seconds", 900)
    ...
    assert calls["runner_config"].loss_streak_cooldown_count == 2
    assert calls["runner_config"].loss_streak_cooldown_seconds == 900
