...
@app.command("one-minute-opening-family-screen")
def one_minute_opening_family_screen(
    fixture: Path = typer.Option(..., "--fixture"),
    output: Path = typer.Option(..., "--output"),
):
    """Run broker-free opening-state family screening on sanitized evidence."""
    from tradingagents.agents.price_action.opening_state_family import (
        screen_family_fixture,
    )

    report = screen_family_fixture(fixture)
...
@app.command("one-minute-opening-target-grid-screen")
def one_minute_opening_target_grid_screen(
    fixture: Path = typer.Option(..., "--fixture"),
    output: Path = typer.Option(..., "--output"),
):
    """Run broker-free opening-state target-grid screening."""
...
@app.command("one-minute-v10-screen")
def one_minute_v10_screen(
    fixture: list[Path] = typer.Option(..., "--fixture"),
    manifest: Path = typer.Option(..., "--manifest"),
    output: Path = typer.Option(..., "--output"),
    stage: str = typer.Option("DISCOVERY", "--stage"),
...
@app.command("one-minute-v10-register-prospective")
def one_minute_v10_register_prospective(
    manifest: Path = typer.Option(..., "--manifest"),
    held_out_report: Path = typer.Option(..., "--held-out-report"),
    output: Path = typer.Option(..., "--output"),
):
    """Start V10's prospective clock after the held-out gate passes."""
    from tradingagents.agents.price_action.one_minute_causal_reclaim_v10_screening import (
        register_v10_prospective,
    )

    try:
        record = register_v10_prospective(
            manifest_path=manifest,
            held_out_report_path=held_out_report,
            output_path=output,
        )
            ...
            runner_config_kwargs.update(
                {
                    ...
                    "reserve_stop_risk_against_session_limit": experimental_requested,
                }
            )
        if trading_mode == TradingMode.AUTO_GATED:
